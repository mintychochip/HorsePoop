...
public class MatchingNonConservedStep<U extends TraitEnum> implements GenomeCrossingStep<U> {
    @Override
    public List<BaseTrait<U>> crossTraits(List<BaseTrait<U>> father, List<BaseTrait<U>> mother, EntityType entityType, List<BaseTrait<U>> baseTraits, TraitConfig<U> config, Fetcher<U> fetcher, TraitCrosser<U> traitCrosser) {
        List<BaseTrait<U>> collect = father.stream()
                .filter(trait -> !trait.getMeta().isConserved())
                .filter(trait -> fetcher.getTraitFromList(mother, trait.getMeta().getTrait()) != null)
                .toList();
        return collect.stream().map(trait -> traitCrosser.crossTraits(trait,fetcher.getTraitFromList(mother,trait.getMeta().getTrait()))).collect(Collectors.toList());
