...
public class InstanceConserved<U extends TraitEnum> implements InstancingStep<U> {
  ...
  public List<BaseTrait<U>> instanceTrait(EntityType entityType, List<BaseTrait<U>> baseTraits,
    ...
    return config.getAllEntityTraits(entityType).stream() //all traits for trait type U
            .filter(trait ->config.getMeta(trait,entityType) instanceof Conserved c && c.isConserved()) // filter if it is a member of conserved, then continue and instance if it is
